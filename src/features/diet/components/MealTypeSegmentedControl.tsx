...
export function MealTypeSegmentedControl({
  value,
  onChange,
  includeAll = false,
}: {
  ...
  const values: Array<MealType | 'all'> = includeAll
    ? ['all', 'breakfast', 'lunch', 'dinner', 'snack', 'other']
    ...
    <View style={styles.wrap}>
      {values.map(item => (
        <Pressable
          ...
          style={[styles.item, item === value && styles.active]}
        >
          <Text style={[styles.text, item === value && styles.activeText]}>
            {item === 'all' ? '全部' : mealTypeLabels[item]}
          </Text>
        </Pressable>
      ))}
    </View>
