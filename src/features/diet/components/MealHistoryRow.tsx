...
export function MealHistoryRow({ meal, onPress }: { meal: MealHistoryItem; onPress: () => void }) {
  ...
  const status = meal.analysisStatus === 'completed' ? `${energy?.amount ?? 0} kcal` : meal.analysisStatus === 'failed' ? 'AI 分析失败' : meal.analysisStatus === 'analysing' ? 'AI 分析中' : '等待 AI 分析';
  return <Pressable accessibilityRole="button" onPress={onPress}><GlassCard style={styles.card}><Text style={styles.emoji}>{emoji[meal.mealType]}</Text><View style={styles.copy}><Text numberOfLines={1} style={styles.title}>{meal.notes || mealTypeLabels[meal.mealType]}</Text><Text style={styles.meta}>{new Date(meal.consumedAt).toLocaleString([], { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })} · {status}</Text></View><ChevronRight color={colors.muted} size={17} /></GlassCard></Pressable>;
}
