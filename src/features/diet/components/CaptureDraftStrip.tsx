...
export function CaptureDraftStrip({ drafts, selectedId, maxDrafts, disabled = false, onSelect, onDelete, onNew }: Props) {
  ...
  return <View style={styles.wrap}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
      ...
      <Pressable accessibilityRole="button" disabled={disabled || maxDrafts === null || drafts.length >= maxDrafts} onPress={onNew} style={[styles.newCard, (disabled || maxDrafts === null || drafts.length >= maxDrafts) && styles.disabled]}><Plus color={colors.blue} size={20} /><Text style={styles.newText}>新建一餐</Text></Pressable>
    </ScrollView>
    {maxDrafts !== null && drafts.length >= maxDrafts ? <Text style={styles.hint}>草稿已达上限，请提交或取消一份后再新建。</Text> : null}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  card: { width: 130, padding: 7, borderRadius: 14, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, gap: 6, position: 'relative' },
  selected: { borderColor: colors.blue, borderWidth: 2 },
  deleteButton: { position: 'absolute', top: 7, right: 7, zIndex: 1, width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface },
  thumb: { width: '100%', height: 72, borderRadius: 9, backgroundColor: colors.surfaceMuted },
  placeholder: { width: '100%', height: 72, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceMuted },
  ...
  newCard: { width: 130, height: 108, borderRadius: 14, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.blueSoft, alignItems: 'center', justifyContent: 'center', gap: 5 },
