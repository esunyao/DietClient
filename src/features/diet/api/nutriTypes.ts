...
export interface CapturePolicy {
  ...
  allowedContentTypes: ImageContentType[];
  sessionExpiresInSeconds: number;
  maxDraftSessionCount: number;
...
export interface CaptureDraftSummary {
  captureSessionId: CaptureSessionId;
  status: Extract<CaptureStatus, 'created' | 'uploading'>;
...
export interface CaptureDraftPage {
  items: CaptureDraftSummary[];
  total: number;
}
...
export interface CaptureImagePresignRequest {
  fileName: string;
  contentType: ImageContentType;
  contentLength: number;
  capturedAt?: string | null;
}
export interface PresignedCaptureUpload {
  imageId: CaptureImageId;
  uploadUrl: string;
  objectKey: string;
...
export interface MealCorrectionItem {
  itemId?: MealItemId | null;
  displayName: string;
  estimatedWeightG?: number | null;
  notes?: string | null;
  nutrients: Array<{ nutrientCode: string; amount: number }>;
}
...
export interface MealMetadataPatchRequest {
  mealType?: MealType;
  consumedAt?: string;
  timezone?: string;
  notes?: string | null;
