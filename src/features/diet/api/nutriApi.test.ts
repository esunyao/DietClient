...
describe('NutriMemo photo capture contract', () => {
  ...
  it('keeps snowflake and presigned image IDs as strings', async () => {
    ...
    const result = await nutriApi.presignCaptureImage(snowflakeId, {
      fileName: 'plate.jpg',
    ...
    expect(postMock).toHaveBeenCalledWith(
      `v1/nutri/capture-sessions/${snowflakeId}/images/presign`,
      expect.objectContaining({ fileName: 'plate.jpg' }),
    );
  });
  it('sends idempotency key when creating a session', async () => {
    postMock.mockResolvedValue({ data: { data: { captureSessionId: snowflakeId } } });
    await nutriApi.createCaptureSession(
      { timezone: 'Asia/Shanghai' },
      '98fcaaf7-7a6e-44c2-98d0-57ddbc315346',
    );
    expect(postMock).toHaveBeenCalledWith(
      'v1/nutri/capture-sessions',
      { timezone: 'Asia/Shanghai' },
  ...
  it('confirms by imageId and replaces a whole meal without foodId', async () => {
    postMock.mockResolvedValue({ data: { data: { imageId: snowflakeId } } });
    ...
    await nutriApi.replaceMeal(snowflakeId, {
      mealType: 'lunch',
      consumedAt: '2026-08-17T04:00:00.000Z',
      timezone: 'Asia/Shanghai',
      items: [
        {
          displayName: '鸡胸肉',
          estimatedWeightG: 120,
          nutrients: [{ nutrientCode: 'PROTEIN', amount: 31 }],
        },
      ],
    });
    expect(postMock).toHaveBeenCalledWith(
      `v1/nutri/capture-sessions/session-id/images/${snowflakeId}/confirm`,
    );
    expect(putMock).toHaveBeenCalledWith(
      `v1/nutri/meals/${snowflakeId}`,
      expect.not.objectContaining({ foodId: expect.anything() }),
    );
  });
  it('submits meal metadata and patches metadata without replacing items', async () => {
    postMock.mockResolvedValue({
      data: {
        data: {
          captureSession: { status: 'ready_for_analysis' },
          meal: { mealId: snowflakeId, analysisStatus: 'queued' },
        },
      },
    });
    ...
    expect(postMock).toHaveBeenCalledWith('v1/nutri/capture-sessions/session-id/submit', {
      mealType: 'dinner',
      notes: '少油',
    });
