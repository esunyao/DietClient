...
export async function uploadAndConfirmCaptureImage(sessionId: CaptureSessionId, file: CaptureImageFile, onProgress: CaptureUploadProgress): Promise<CaptureSession> {
  if (!Number.isFinite(file.byteSize) || file.byteSize <= 0) throw new Error('无法读取图片大小，请重新选择。');
  const presign = await nutriApi.presignCaptureImage(sessionId, { fileName: file.fileName, contentType: file.contentType, contentLength: file.byteSize, capturedAt: file.capturedAt });
