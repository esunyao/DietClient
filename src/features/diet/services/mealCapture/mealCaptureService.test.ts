jest.mock('../../api/nutriApi', () => ({
  nutriApi: {
    ...
    listCaptureDrafts: jest.fn(),
    getCaptureSession: jest.fn(),
    presignCaptureImage: jest.fn(),
    confirmCaptureImage: jest.fn(),
    cancelCaptureSession: jest.fn(),
    submitCaptureSession: jest.fn(),
    retryCaptureSession: jest.fn(),
  },
}));
jest.mock('./captureSessionStorage', () => ({
  readActiveCaptureSessionId: jest.fn(),
  saveActiveCaptureSessionId: jest.fn(),
  clearActiveCaptureSessionId: jest.fn(),
}));
...
} from './mealCaptureService';
import { uploadCaptureImageBinary } from './mealCaptureUpload';

const session = {
  captureSessionId: '2086475596958904300',
  status: 'created',
...
describe('meal capture service', () => {
  ...
  it('uses presigned imageId for ordered upload then confirmation', async () => {
    (nutriApi.presignCaptureImage as jest.Mock).mockResolvedValue({
      imageId: '2086475596958904301',
      uploadUrl: 'https://upload',
      requiredHeaders: {},
    });
    (nutriApi.getCaptureSession as jest.Mock).mockResolvedValue(session);
    await uploadAndConfirmCaptureImage(
    ...
    expect((uploadCaptureImageBinary as jest.Mock).mock.invocationCallOrder[0]).toBeLessThan(
      (nutriApi.confirmCaptureImage as jest.Mock).mock.invocationCallOrder[0],
    );
    expect(nutriApi.confirmCaptureImage).toHaveBeenCalledWith(
      session.captureSessionId,
      '2086475596958904301',
    );
  });
  it('submits the selected meal metadata', async () => {
    const response = {
      ...
      meal: { mealId: '2086475596958904302', analysisStatus: 'queued' },
    };
    (nutriApi.submitCaptureSession as jest.Mock).mockResolvedValue(response);
    await expect(
      submitCaptureSession(session.captureSessionId, { mealType: 'lunch', notes: '少盐' }),
    ).resolves.toEqual(response);
    expect(nutriApi.submitCaptureSession).toHaveBeenCalledWith(session.captureSessionId, {
      mealType: 'lunch',
      notes: '少盐',
    });
  });
