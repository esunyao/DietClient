...
import {
  makeCaptureFileName,
  prepareCaptureMimeType,
  type CaptureImageFile,
  type CaptureUploadProgress,
} from './mealCapture.types';

/** Web/测试回退实现；原生运行时由同名 .native 文件覆盖。 */
export async function prepareCaptureImageFile(asset: Asset): Promise<CaptureImageFile> {
  ...
  const contentType = prepareCaptureMimeType({
    ...asset,
...
export async function uploadCaptureImageBinary(
  ...
  await uploadBlob(
    file.blob ?? (await fetchFileBlob(file.uri)),
    uploadUrl,
    { ...requiredHeaders, 'Content-Type': file.contentType },
    onProgress,
    '餐食图片上传失败',
    '上传被浏览器跨域策略或网络拦截，请确认对象存储已开启 CORS。',
  );
}
