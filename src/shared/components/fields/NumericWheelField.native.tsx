...
export function NumericWheelField({
  label,
  value,
  onChange,
  minimum,
  ...
  unit,
  placeholder,
}: NumericWheelFieldProps) {
  const [opening, setOpening] = useState(false);
  const number = Number(value);
  const selected =
    value.trim() !== '' && Number.isFinite(number) && number >= minimum && number <= maximum
      ? number
      : null;
  ...
  const open = useCallback(async () => {
    ...
    try {
      ...
      if (result.action === 'confirm' && result.value != null)
        onChange(result.value.toFixed(digits));
    } finally {
      setOpening(false);
    }
  ...
  const display =
    selected == null
      ...
      : `${selected.toFixed(digits)}${unit ? ` ${unit}` : ''}`;
  return (
    <View style={styles.field}>
      ...
      <Pressable
        ...
        disabled={opening}
        onPress={open}
        style={[styles.control, opening && styles.disabled]}
      >
