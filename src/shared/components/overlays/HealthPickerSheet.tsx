...
export function HealthPickerSheet({
  visible,
  title,
  value,
  options,
  ...
  useEffect(() => {
    if (visible) {
      setDraft(value);
      progress.value = withTiming(1, timing(durations.sheetIn));
    }
  ...
  const close = () => {
    progress.value = withTiming(0, timing(durations.sheetOut));
    onCancel();
  };
  return (
          ...
          <GlassSurface variant="navigation" style={styles.panel}>
            ...
            <View style={styles.heading}>
              <View>
                ...
                <Text style={styles.subtitle}>选择最符合当前情况的一项</Text>
              </View>
              ...
              <View style={styles.options}>
                {options?.map(option => (
                  <Pressable
                    ...
                    style={[styles.option, draft === option.value && styles.optionActive]}
                  >
                    <View style={styles.optionCopy}>
                      <Text
                        style={[
                          styles.optionText,
                          draft === option.value && styles.optionTextActive,
                        ]}
                      >
                        {option.label}
                      </Text>
...
export function HealthSelectField({
  label,
  value,
  options,
  onChange,
      ...
      >
        <View style={styles.valueCopy}>
          ...
          {selected?.description ? (
            <Text numberOfLines={1} style={styles.valueDescription}>
              {selected.description}
            </Text>
          ) : null}
        </View>
        <ChevronDown color={colors.blue} size={19} />
      </Pressable>
      <HealthPickerSheet
